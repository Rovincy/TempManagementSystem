...
import {Api_Endpoint, fetchPaymentMethods} from '../../../../../services/ApiCalls'
...
const PaymentMethod = () => {
  ...
  const {data: paymentMethodData, isLoading: paymentMethodLoad} = useQuery('paymentMethod', fetchPaymentMethods)
  ...
  const deleteData = async (element: any) => {
      ...
      onOk: async () => {
          const response = await axios.delete(`${BASE_URL}/paymentMethod/id?id=`+element)
  ...
  const columns: any = [
...

    {
      ...
      render: (_: any, record: any) => (
        <Space size='middle'>
          <Link to={`/paymentMethodEditForm`} state={record}>
    ...
    >
      <KTCardBody className='py-4 '>
        ...
        {/* <Link to='/roomType'>
          ...
          <Table
            columns={columns}
            dataSource={paymentMethodData?.data}
            loading={paymentMethodLoad}
